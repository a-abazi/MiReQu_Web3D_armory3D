...
class KM100_Base_CPS532_C2_Laser extends iron.Trait {
	...
	@prop 
	var screwName: String = "KM100_Screw_CPS532_C2_Laser";
	@prop
	var outputName: String = "Laser_Output_CPS532_C2";
	...
	function onInit() {
		...
		if (object.properties["spawned"] == true){
			visib = true;
		}
...
//
		...
		// gives each object in objList the property of the main"Object" and the corresponding "Trait"
		// these properties are used by the global Trait ResumeTraitSelector.hx to "resume" traits
		// (traits are paused after an interaction, see usage of "pauseUpdate();" )
		for (obj in objList){
			initProps(obj);
			if (obj != object) obj.properties.set("TraitObj",object);
			else obj.properties.set("TraitObj","self");
			obj.properties.set("TraitName","KM100_Base_CPS532_C2_Laser");
			obj.properties.set("PauseResume",true);
		}
		pauseUpdate();

		if (object.properties["spawned"] == true){
			var globalObject: Object = iron.Scene.global;
			initProps(globalObject);
			if (globalObject.properties["laserArray"] == null){
				globalObject.properties["laserArray"] = [laser.getChild(outputName)];
			}
			else globalObject.properties["laserArray"].push(laser.getChild(outputName));
		}
