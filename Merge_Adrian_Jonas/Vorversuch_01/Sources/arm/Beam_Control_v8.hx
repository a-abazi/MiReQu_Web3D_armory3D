...
import armory.logicnode.QuaternionNode;
...
class Beam_Control_v8 extends iron.Trait {

	@prop
	var directSpawn: Bool = false;

	@prop 
	var directSpawn_LaserName : String = "LDM56_06";
	...
	var nameArrayDetectableBeams: String = "ArrayDetBeams";
	
	var arrayDetectableBeams: Array<Map<String,Dynamic>> = [new Map()];

	var laserArray: Array<Object> = [];
	...
	function onInit() {
		...
		if (directSpawn) laserArray.push(Scene.active.root.getChild(directSpawn_LaserName));
		if (object.properties["laserArray"] == null){
			object.properties["laserArray"] = laserArray;
		}
	...
	function onUpdate(){
		...
		if (!directSpawn){
			var object: Object = iron.Scene.global;
			if (object.properties["laserArray"]!= null) laserArray = object.properties["laserArray"];
		}
	...
	function onEventCB(){ // Event to calculate Beams and such
		...
		for (laser in laserArray){
			main_Array_pos[indx] = []; //Reset Array
			main_Array_pos[indx].push(laser.transform.world.getLoc());//add first position
			main_Array_dir[indx] = [];
			main_Array_dir[indx].push(laser.transform.world.look());
			main_Array_sor[indx] = [];
			main_Array_sor[indx].push(laser);
			indx++;

			if (main_Array_beams.length< laserArray.length) main_Array_beams.push([]);
			if (main_All_arrows.length< laserArray.length) main_All_arrows.push([]);
		}
		...
		// begin Main loop, limited by rayNumber
				...
				for (parent_index in 1...curr_sources.length){ // skip the first entry as it already spawns the array
					...
					if (source_object.properties.get("spawnsRays") != null){
						...
						for (j in 0...spawnsRays){
							...
							source_object.properties.set("ChildBeam"+Std.string(j), new_Beam);
	...
	function getDetectableBeams(arrays : Array <Dynamic>, rayNumber:Int) {
		...
		currentMap.set("dir",arr_dir[arr_dir.length-1]);
	...
	function spawnBeams(arrays : Array <Dynamic>, blocked):Bool {
        ...
        for (i in 0...arr_pos.length) {
            ...
            else if (i==arr_pos.length-1){ //Special Case for last Beam
                    if (blocked) { // is beam blocked? if yes, function is terminated and last beam removed
                        while(arr_pos.length-1<arr_beams.length){
                            arr_beams.pop().remove();
                        }
                        break;
                    } // if not the beam is "infinetely extended"
                    scale.x = 1000.;
            }
			else {	// other Beams
                var vec = new Vec4();
			...
			var theta = Math.asin(-rot.z);
			// then calculate quaternion for the rotation according to the angles psi theta
			q.fromAxisAngle(z,psi).mult(new Quat().fromAxisAngle(y,theta));
            ...
            if (i==0){
				var fromLaser = false;
				// check if source object is one of the possible laser, i.e. an object in laserArry
				for (thisLaser in laserArray){
					if (source_object == thisLaser) {
						fromLaser = true;
						childprops = callTraitFunction(source_object, Proptraitname, funName_BP, [arr_beams[i], arr_pos[i], arr_dir[i]]);
					} 
				}
				if (!fromLaser) childprops = curr_beam.properties.copy();
			}
            ...
            if (i>0 && source_object.properties.get("changes_prop")){
				...
				// save previous beam and new beam to the source object
				source_object.properties.set("PrevBeam",arr_beams[i-1]);
				source_object.properties.set("NextBeam",arr_beams[i]);
			}
        ...
        if (pol_on==false || laser_on == false){
            ...
            for (i in 0...arr_pos.length) {
                ...
                for (j in 0...numArrows){
					...
					if (singlePolArrow){
						if (i > arr_pos.length-2){
							loc = new Vec4().add(arr_dir[i]);
                   			loc.normalize();
                   			loc.mult((j+1)*arrow_dist);
							loc.add(arr_pos[i]);
						}
						else{
							loc = new Vec4().sub(arr_pos[i]).add(arr_pos[i+1]).mult(0.5).add(arr_pos[i]);
						}
					}
