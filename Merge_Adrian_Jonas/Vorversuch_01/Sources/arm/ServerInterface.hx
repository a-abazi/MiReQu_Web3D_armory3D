...
class ServerInterface extends iron.Trait {
    ...
    @prop
    var serverIP: String = "192.168.2.100:5000";
    ...
    public var currDataTimeTagger:CCJson;
    public var currDataRotEncoder:REJson;
    public var currPhotoDiode:PDJson;
...
//
        ...
        if (virtualInput){
            if (ui.window(Id.handle(), pos_x, pos_y, width, height, true)) {
                ...
                if (ui.panel(Id.handle({selected: false}), "Virtual Input Voltage")) {
                    var voltage1:Float = ui.slider(Id.handle({value: 250}), "Voltage Diode 1", 0., 500., true);
                    var voltage2:Float = ui.slider(Id.handle({value: 250}), "Voltage Diode 2", 0., 500., true);
    ...
    public function exportTMP(expData: Map<String,Dynamic> ):Void{      
