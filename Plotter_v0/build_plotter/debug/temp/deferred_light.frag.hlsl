...
{
    float3 param = lp;
    float _275 = lpToDepth(param, lightProj_1);
    float compare = _275 - (bias * 1.5f);
    ml += ((n * bias) * 20.0f);
    ml.y = -ml.y;
    float4 _295 = float4(ml, compare);
    float result = shadowMapCube.SampleCmp(_shadowMapCube_sampler, _295.xyz, _295.w);
    float4 _307 = float4(ml + 0.001000000047497451305389404296875f.xxx, compare);
    result += shadowMapCube.SampleCmp(_shadowMapCube_sampler, _307.xyz, _307.w);
    float4 _321 = float4(ml + float3(-0.001000000047497451305389404296875f, 0.001000000047497451305389404296875f, 0.001000000047497451305389404296875f), compare);
    result += shadowMapCube.SampleCmp(_shadowMapCube_sampler, _321.xyz, _321.w);
    float4 _334 = float4(ml + float3(0.001000000047497451305389404296875f, -0.001000000047497451305389404296875f, 0.001000000047497451305389404296875f), compare);
    result += shadowMapCube.SampleCmp(_shadowMapCube_sampler, _334.xyz, _334.w);
    ...
    float4 _399 = float4(ml + (-0.001000000047497451305389404296875f).xxx, compare);
...
float3 sampleLight(float3 p, float3 n, float3 v, float dotNV, float3 lp, float3 lightCol, float3 albedo, float rough, float spec, float3 f0, int index, float bias)
{
    ...
    direct *= lightCol;
    float3 param = -l;
    float _455 = PCFCube(shadowMapPoint[0], _shadowMapPoint_sampler[0], ld, param, bias, lightProj, n);
    direct *= _455;
    return direct;
}
...
void frag_main()
{
    ...
    {
        _610 = octahedronWrap(g0.xy);
    }
    n = float3(_610.x, _610.y, n.z);
    n = normalize(n);
    ...
    float3 _725 = fragColor.xyz * ssaotex.SampleLevel(_ssaotex_sampler, texCoord, 0.0f).x;
    fragColor = float4(_725.x, _725.y, _725.z, fragColor.w);
    int param_2 = 0;
    float param_3 = pointBias;
    float3 _748 = fragColor.xyz + sampleLight(p, n, v, dotNV, pointPos, pointCol, albedo, roughness, occspec.y, f0, param_2, param_3);
    fragColor = float4(_748.x, _748.y, _748.z, fragColor.w);
    fragColor.w = 1.0f;
}
