...
class Beam_Control_v6 extends iron.Trait {

	@prop 
	var pump_laser_name : String = "LDM56_06";
	
	@prop 
	var pump_laser_parentName : String = "LDM56_Base";
	...
	function onInit() {
		...
		if (!directSpawn){
			pump_laser_parent = Scene.active.root.getChild(pump_laser_parentName);
		}
	}
	...
	function onEventCB(){ // Event to calculate Beams and such
		...
		main_Array_dir[0] = [];
		main_Array_dir[0].push(pump_laser.transform.world.look());
		main_Array_sor[0] = [];
