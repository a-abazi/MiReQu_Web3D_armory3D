...
class Beam_Control_v5 extends iron.Trait {
	...
	function spawnBeams(arrays : Array <Dynamic>, blocked):Bool {
        ...
        for (i in 0...arr_pos.length) {
            ...
            if (i==0){
				if (source_object == pump_laser){
					childprops = callTraitFunction(source_object, Proptraitname, funName, [arr_beams[i], arr_pos[i], arr_dir[i]]);
					childprops_names = childprops[0];
					childprops_values = childprops[1];
				}
				else{
					childprops_names = callTraitFunction(source_object, Proptraitname, funName_BP_Names, []);
					var parBeam_props = arr_beams[i].properties;
					for (name in childprops_names) childprops_values.push(parBeam_props[name]);
				}
            }
            ...
            if (i>0){
                ...
                if (source_object.properties.get("changes_prop")){
					...
					var new_childprops = callTraitFunction(source_object, Proptraitname, funName, [arr_beams[i-1], arr_pos[i], arr_dir[i]]);
	...
	function polControl(arrays : Array <Dynamic>, arr_all_arrows: Array<Object>):Bool {	
        ...
        if (pol_on==false || laser_on == false){
            ...
            for (i in 0...arr_pos.length) {
				...
				if ((Math.abs(stokes_chi))>0) continue;
				
				var pol_amp = new Vec4(0,Math.cos(stokes_psi),Math.sin(stokes_psi),1);
				pol_amp.normalize();
