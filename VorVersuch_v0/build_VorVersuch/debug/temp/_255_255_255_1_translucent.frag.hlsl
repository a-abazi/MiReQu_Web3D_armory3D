...
{
    float3 _198 = float3(uv + ((-1.0f).xx / smSize), compare);
    float result = shadowMap_1.SampleCmp(_shadowMap_1_sampler, _198.xy, _198.z);
    float3 _207 = float3(uv + (float2(-1.0f, 0.0f) / smSize), compare);
    ...
    float3 _229 = float3(uv + (float2(0.0f, -1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _229.xy, _229.z);
    float3 _237 = float3(uv, compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _237.xy, _237.z);
    float3 _248 = float3(uv + (float2(0.0f, 1.0f) / smSize), compare);
    ...
    float3 _270 = float3(uv + (float2(1.0f, 0.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _270.xy, _270.z);
    float3 _281 = float3(uv + (1.0f.xx / smSize), compare);
...
float shadowTestCascade(Texture2D<float4> shadowMap_1, SamplerComparisonState _shadowMap_1_sampler, float3 eye_1, float3 p, float shadowsBias_1)
{
    ...
    int param;
    int param_1;
    float4x4 _410 = getCascadeMat(d, param, param_1);
    int casi = param;
    int casIndex = param_1;
    float4x4 LWVP = _410;
    float4 lPos = mul(float4(p, 1.0f), LWVP);
    float3 _425 = lPos.xyz / lPos.w.xxx;
    lPos = float4(_425.x, _425.y, _425.z, lPos.w);
    ...
    if ((splitDist <= 0.1500000059604644775390625f) && (casi != 3))
    {
        ...
        float3 _528 = lPos2.xyz / lPos2.w.xxx;
        lPos2 = float4(_528.x, _528.y, _528.z, lPos2.w);
...
float3 sampleLight(float3 p, float3 n, float3 v, float dotNV, float3 lp, float3 lightCol, float3 albedo, float rough, float spec, float3 f0, int index, float bias, bool receiveShadow_1, bool isSpot, float spotA, float spotB, float3 spotDir_1)
{
    float3 ld = lp - p;
...
void frag_main()
{
    ...
    bool param_3 = true;
    float param_4 = spotData.x;
    float param_5 = spotData.y;
    float3 param_6 = spotDir;
    direct += sampleLight(wposition, n, vVec, dotNV, pointPos, pointCol, albedo, roughness, specular, f0, param, param_1, param_2, param_3, param_4, param_5, param_6);
