...
// UPH v2 Works with different post and holder heights
...
class UPH_Base extends iron.Trait {	

	@prop 
	var plateName: String = "Platte";

	@prop 
	var mScrewName: String = "UPH_M6_Screw";
	@prop 
	var screwName: String = "UPH_Screw";
	@prop 
	var postName: String = "UPH_TR";
	@prop 
	var topName: String = "UPH_Top";
	...
	public function onInit() {
		...
		if (object.properties["spawned"]){
			visib = true;
			postName = (object.properties["postName"]);
			topName = (object.properties["topName"]);
			sLoc = object.properties["sLoc"];
		}
		else {
			removeUpdate(onUpdate);
			return;
		...
		object.transform.translate(0,0,zero.z -  object.getChildren()[0].transform.world.getLoc().z); // Child should be: 00_C_Platte_UPH_Base
		...
		var mST = object.getChildren()[3].transform.world; // Child should be : 03_C_Screw_Pos_UPH_Base
		mScrew.transform.setMatrix(mST);
		mScrew.transform.world.setLoc(sLoc);
		...
		var mT = object.getChildren()[4].transform.world; // Child; 04_C_UPH_UPH_Base
		top.transform.setMatrix(mT);
		top.transform.scale = nScale;
		rbSync(top);
		corrVTop = new Vec4().setFrom(top.getChildren()[0].transform.world.getLoc()).sub(top.transform.loc); // child shoud be 00_C_Bottom_UPH_TopXX
		...
		var mS = top.getChildren()[1].transform.world;
		screw.transform.setMatrix(mS);
		screw.transform.scale = nScale;
		rbSync(screw);
		corrScrew = new Vec4().setFrom(screw.getChildren()[0].transform.world.getLoc()).sub(screw.transform.loc);
		if (sState == 0)
			corrScrew.mult(0.5);
		...
		var mP = top.getChildren()[2].transform.world;
		...
		if (object.properties["emptySlot"]!=null){
			if (object.properties["emptySlot"]) post.visible = false;
		}

		
		// measure limits from the empty child objects, limits for postTravel
		postPosLim = (post.getChildren()[1].transform.world.getLoc().distanceTo( top.getChildren()[2].transform.world.getLoc()))-0.09;
		postNegLim = (post.getChildren()[0].transform.world.getLoc().distanceTo( top.getChildren()[2].transform.world.getLoc())); 
		var postNegLimAlternative = (post.getChildren()[1].transform.world.getLoc().distanceTo( top.getChildren()[0].transform.world.getLoc()))+0.0109;
		if (postNegLimAlternative<postNegLim) postNegLim = postNegLimAlternative;
		// TODO: Fix Limits for Long Posts and short holder

		/* For debugging
		trace("PosLim"+ Std.string(postPosLim));
		trace("NegLim"+ Std.string(postNegLim));

		trace("PostTop "+Std.string(post.getChildren()[1].transform.world.getLoc()));
		trace("PostBot "+Std.string(post.getChildren()[0].transform.world.getLoc()));
		trace("HolderTop "+ Std.string(top.getChildren()[2].transform.world.getLoc()));
		*/
		...
		basePosLim = (object.getChildren()[3].transform.world.getLoc().distanceTo( object.getChildren()[2].transform.world.getLoc()));
		baseNegLim = (object.getChildren()[3].transform.world.getLoc().distanceTo( object.getChildren()[1].transform.world.getLoc())); 
		baseTravelDist = Math.abs(basePosLim) + Math.abs(baseNegLim);


		

		//trace( "given postHeight" + postHeight);
		//trace( "Actual postHeight"+ Std.string((post.getChildren()[0].transform.world.getLoc())));

		postDist = postHeight - post.getChildren()[0].transform.world.getLoc().z ;
		//trace("calc post Dist" + Std.string(postDist));
		...
		if (object.properties["componentObjectName"] != null ){
			...
			var mCC = post.getChildren()[0].transform.world;
		...
		//trace( "new postHeight"+ Std.string((post.getChildren()[0].transform.world.getLoc())));
		...
		for (obj in objList){
			...
			obj.properties.set("TraitName","UPH_Base");
		...
		*/
		...
		if (mouse.down("left") && movingObj != null ){
			...
			// the base is rotated according to the mouseposition and translated with fixed screw
				...
				if (newHitVec!= null ) {
					var dirVecNew = new Vec4().setFrom(newHitVec).sub(object.getChildren()[3].transform.world.getLoc());
					var newAngle = Math.atan2(dirVecNew.y,dirVecNew.x)-Math.atan2(1,0);
					rotBaseTo(newAngle);
					
					var distold = object.getChildren()[3].transform.world.getLoc().distanceTo(hitVec);
					var distnew = object.getChildren()[3].transform.world.getLoc().distanceTo(newHitVec);
	...
	function onRemove(){
		...
		if (object.properties["componentObjectName"] != null ) comp.remove();
	}
	...
	function updateParts() {
		...
		var mST = object.getChildren()[3].transform.world;
		mScrew.transform.setMatrix(mST);
		mScrew.transform.scale = nScale;
		rbSync(mScrew);
		...
		if (mState == 0 || mState ==1 ){
			...
			object.transform.rot.fromAxisAngle(object.transform.up().normalize(), baseAngle);
			rbSync(object);
			var corrVrot = new Vec4().setFrom(object.getChildren()[3].transform.world.getLoc());
		...
		var mT = object.getChildren()[4].transform.world;
		top.transform.setMatrix(mT);
		top.transform.scale = nScale;
		top.transform.loc.sub(corrVTop);
		rbSync(top);
	...
	function transBase(multiplier: Float){
		...
		if (mState==1){
			...
			if (multiplier<0){
				var baseDist = (object.getChildren()[3].transform.world.getLoc().distanceTo( object.getChildren()[1].transform.world.getLoc()));
				if (baseDist + multiplier*baseTrans*scalefactor>0){
					trace(baseDist);
					trace(multiplier*baseTrans);
					object.getChildren()[3].transform.translate(0,multiplier*baseTrans,0);
					rbSync(object);
				}
			}
			else if(multiplier>0){			
				var baseDist = (object.getChildren()[3].transform.world.getLoc().distanceTo( object.getChildren()[2].transform.world.getLoc())); 
				if (baseDist - multiplier*baseTrans*scalefactor >0){
					trace(baseDist);
					object.getChildren()[3].transform.translate(0,multiplier*baseTrans,0);
					rbSync(object);
	...
	// translates base in relation to screw by a value dist, locked to values by the C_points
		if (mState==1 || ignoreState){
			...
			var baseDist = 0.;
			if (dist<0){
				baseDist = (object.getChildren()[3].transform.world.getLoc().distanceTo( object.getChildren()[1].transform.world.getLoc()));
			}
			else if(dist>0){			
				baseDist = (object.getChildren()[3].transform.world.getLoc().distanceTo( object.getChildren()[2].transform.world.getLoc())); 	
			}
			if (baseDist - Math.abs(dist)>0){
				object.getChildren()[3].transform.translate(0,dist*scalefactor,0);
				object.transform.move(object.transform.look().normalize(),-1*dist);
				rbSync(object);			
			}	
		allVariablesToProbs(object);
		updateParts();	
	...
	function rbSync(object:Object):Bool { 
	...
	inline function allPropsToVariables(object:Object){
		...
		postHeight  = object.properties["postHeight"];
	}
	...
	function allVariablesToProbs(object:Object){
		...
		object.properties["postHeight"] = postHeight;
	}
